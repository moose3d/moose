...
namespace Phoenix 
{
  namespace Math 
  {
	...
	CMatrix4x4 mOrig;    // The matrix whose inverse we want to take
	CMatrix4x4 mInverse; // initialize to 4x4 identity matrix
	CMatrix4x4::Identity(mInverse);
	...
	for (j = 0; j < 4; ++j){
	  ...
	  {
	    std::cerr << "Matrix inverse does not exist!" 
				      << std::endl;
