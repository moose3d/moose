...
Moose::AI::CAIScript::Update(  float fSeconds  )
{
  ...
  int retcode = Tcl_Eval(m_pInterp, "Update");
  if ( retcode != TCL_OK )
  {
    g_Error << "In " << m_pEntity->GetScript() << " Update{} : ";
    g_Error << "\n------\n";
    /* Get stack dump */
    Tcl_Obj *options = Tcl_GetReturnOptions(m_pInterp, retcode);
    Tcl_Obj *key = Tcl_NewStringObj("-errorinfo", -1);
    Tcl_Obj *stackTrace;
    Tcl_IncrRefCount(key);
    Tcl_DictObjGet(NULL, options, key, &stackTrace);
    Tcl_DecrRefCount(key);
    g_Error << Tcl_GetString(stackTrace) << "\n-----\n";
  }
  ...
  while( !m_lstProcCalls.empty()  )
  {
    ...
    int retcode = Tcl_Eval( m_pInterp, pProcCall->msg.c_str());
    if (  retcode != TCL_OK /*&&
...
Moose::AI::CAIScript::ReloadScript( const char * szScript )
{
  ...
  Tcl_Eval(m_pInterp, "proc OnSelect {} {\n\n}\nproc OnUnselect {} {\n\n}\n");
  ...
  status = Tcl_Eval(m_pInterp, "Start");
  if ( status != TCL_OK )
  {

    g_Error << "In " << m_pEntity->GetScript() << " Start{} : ";
    g_Error << "\n------\n";
    /* Get stack dump */
    Tcl_Obj *options = Tcl_GetReturnOptions(m_pInterp, status);
    Tcl_Obj *key = Tcl_NewStringObj("-errorinfo", -1);
    Tcl_Obj *stackTrace;
    Tcl_IncrRefCount(key);
    Tcl_DictObjGet(NULL, options, key, &stackTrace);
